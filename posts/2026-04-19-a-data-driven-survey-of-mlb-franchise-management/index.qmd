...
::: {.callout-note}
This article brings the MLB franchise analysis onto the main Redwall Analytics site using the published narrative and chart assets from the standalone [MLB franchise analysis microsite][site]. The full reproducible pipeline still lives in the companion [GitHub repository][repo], where the charts are generated from `lahmanTools`, DuckDB, and supporting baseball data.
:::

## Using `{lahmanTools}` and the MotherDuck MCP with GitHub Copilot CLI to put analysis on steroids

The `lahmanTools` package was created to fill in some of the gaps in the already outstanding [Lahman baseball database](https://sabr.org/lahman-database/), such as WAR and recent salary data, to allow a more complete analysis of franchise management over the period since 1985, where the data is most complete.
