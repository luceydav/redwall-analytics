...
In his Yelp blog post, Bob Rudis used "root.dfs" as the path to the Yelp tables. At first, we didn't understand what this referred to, but it is used as the path to the root of the file system where the data is stored as configured in the storage plug-ins. The "Storage" page of the Drill Web App is in Figure \@ref(fig:storage). Both his and the Apache documentation also refer the "cp" path to refer to example JAR data in the Drill "classpath". In addition to the two defaults, all the plug-ins available for hive, mongo, s3, kafka, etc. are also shown below. 

```{r 'storage', echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE, screenshot.opts = list(delay = 20), fig.cap='Drill Web App Storage', out.height='100%'}
knitr::include_url('http://localhost:8047/storage')
```

By clicking on the "Update" button for "dfs", it is easy to modify the "workspace", "location" and "defaultInputFormat" with the path to the file with your data as shown in Figure \@ref(fig:dfs). In our case, we changed the name of workspace to "root", the location to "/Volumes/davidlucey/aDL/data/yelp_dataset/" and the defaultInputFormat to "json". All the different data types are shown further down in "formats", which is one of the big selling points. According to `{sergeant}`, it is possible to even combine disparate source types like: json, csv, parquet and rmdbs by modifying formats when configuring "dfs", while pointing to almost any distributed file system. Once a path is configured in the plug-in, the data in that folder is all set to be queried from RStudio.

```{r 'dfs', echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE, screenshot.opts = list(delay = 20), fig.cap='Drill Web App Storage DFS Panel', out.height='100%'}
...
After setting up the connection, the `{sergeant}` manual returned a message with the current Drill version, but ours showed a Drill version of "00.00.0000", so that might be part of to problem. We can see that connecting to the tables with ODBC took almost twice as long as with the `dplyr` connection, so it seems like we are doing something wrong. When we tried this with Zookeeper, it took 46 seconds, while as shown below, 36 seconds with "Direct to Drillbit".
...
As we go along making queries, everything is collected in the Web App Profiles page, as shown in Figure \@ref(fig:profiles). Clicking on a query here takes us to the Query and Planning page, shown in Figure \@ref(fig:query-and-planning). There are other dashboards which we will show below.

```{r 'profiles', echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE, screenshot.opts = list(delay = 20), fig.cap='Drill Web App Query Profiles', out.height='100%'}
...
The other really interesting thing in Drill was profiling. Here is a more complicated query we experimented with with a couple of joins and some aggregations for a query which wound up taking over an hour. See that we `CAST` the integer variables in this case as we were warned above, but that also didn't seem to make a difference. We are not running this here because it took over an hour to run, but the timing can be seen in Figure \@ref(fig:query-and-planning) further down at 1h11.
...
It is amazing how much information about the query Drill gives us, shown in the Figure \@ref(fig:query-and-planning). Clicking on the "Edit Query" tab, and scrolling down to Operator Profiles, we can see that our attempt at joining almost 10 million rows, twice, ate up over 70% of the query time. A couple of aggregations also cost almost 20%. The scan operators spent time waiting for data and some operators spilled to disc. Even without those bottlenecks, we probably still wouldn't have been satisfied with this amount of time. Having this information, it seems like it would be possible to optimize, but we didn't know how to do it. We have been recently learning SQL and realize that there is still a lot to learn. 


```{r 'query-and-planning', echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE, screenshot.opts = list(delay = 20), fig.cap='Drill Query and Planning Pane for More Complicated SQL Query', out.height='100%'}
knitr::include_url('http://localhost:8047/profiles/20746009-dec6-a077-f88a-79071cac9b86')
```


Lastly, Drill has a nice dashboard which allowed us to for example instruct the hash joins and hash aggregations to ignore memory limits as shown in Figure \@ref(fig:options). There were a lot of parameter settings available, but we were not sure how to adjust these to solve our specific problems, but would welcome any good advice or pointers.


```{r 'options', echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE, screenshot.opts = list(delay = 20), fig.cap='Drill Web App Options', out.height='100%'}
