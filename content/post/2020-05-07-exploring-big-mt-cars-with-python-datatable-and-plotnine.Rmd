...
# Separating and Generating Variables
...
# Using Regular Expressions for Row Operations
...
In the chunk (below), we show how to select columns from the big_mt names tuple by creating the `measures` selector using regex matches the key identifier columns and for integer mileage columns matching '08'. Again, this seemed more complicated and we couldn't do it in line within the frame as we would have with .SD = patterns(). We also wanted to include the identifier columns like `year`, `make` and `model` on the left side of the table, but couldn't find a equivalent `setcolorder` function. There is documentation about multi-column selection, but we couldn't figure out how to make it work. We show the frame with the `year_filter` which we set up earlier. 
