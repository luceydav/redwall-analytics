...
lapply(c("bs4", "requests"), function(package) {
...
In late May, Redwall recently visited its favorite family beach vacation spot, and was thinking that it was so nice, it would be fun go back again in August. We would like to know all of the available choices for the week without having to scroll through incomplete realtor websites. Maybe we would also like to monitor availability and prices on an ongoing basis going forward. Now that we are all set up with Python via `reticulate`, this seems like an opportunity to add a few new skills. When we started this mini-project, we hoped to use `datatable` as our main data frame in conjunction with the Python libraries like `BeautifulSoup` and data structures like dictionaries, but we soon learned that `datatable` doesn't support dates yet. In [Exploring Big MT Cars with Python datatable-Part 1](https://redwallanalytics.com/2020/05/07/exploring-big-mt-cars-with-python-datatable-and-plotnine-part-1/), we had worked around the lack of reshaping capability and the inability to pipe into plots, but this really a deal breaker. As a result, though we were able to keep using `plotnine`, we were finally forced to get better with `pandas`.
...
Again, we wanted to avoid re-running the code, we are showing our saved data from disc. We load the saved data from our text file as a list of 1232 Python dictionaries. The dictionary for a sample listing of '464' is shown in the chunk below. The attributes of the listing are deeply nested and not easy to filter and sort. However, we learned that it is easy to extract the desired elements using the dictionary keys, which we do in the `get_calendar` function below.
