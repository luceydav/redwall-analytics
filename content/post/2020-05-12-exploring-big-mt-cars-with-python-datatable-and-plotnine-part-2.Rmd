...
It is also important to mention that confidence intervals are not implemented yet for lowess smoothing in `plotnine`. This probably isn't such a big deal for our purposes in this graph where there are a large number of models, but it does detract from Figure \@ref(fig:ev-vs-gas-powered) below, where it the uncertainty about the true mean efficiency of cars with batteries in the early years is high because there were so few models.
