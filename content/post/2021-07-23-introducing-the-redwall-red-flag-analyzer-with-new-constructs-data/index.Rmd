...
-   The [High Valuation]{.ul} "red flag" was calculated using New Construct's 1-5 ratings, themselves derived variables, rather than raw valuation metrics. New Constructs does not use the traditional GAAP earnings and book value ratios to determine valuation, for reasons very well discussed in the [Basic Metrics](https://www.newconstructs.com/education/basic-metrics/) section on New Construct's website.

-   We don't know what Mr. Gulliver would have used, but the cutoff for "high" [Earnings Distortion]{.ul} was set for cases simultaneously with greater than 1% of assets [and]{.ul} 10% of earnings, which varied over the time period, but yielded "red flags" in 12% of filings.
...
The beauty of our Shiny app is that we are able leave it for the user to explore the inputs to test sensitivity of "red flags" to performance for themselves. No locked analysis with p-values here, and the "red flags" (not New Construct's raw data) will be available for others to test. In aggregate considering all periods, we observe that median group returns tend to be more stable and higher when the number of "red flags" is below 2-3. There are naturally a lot fewer filings in higher flag groups, and so as mentioned earlier, aggregate returns become considerably more volatile with a higher number of flags and lower confidence can be attributed to those estimates. The separation among groups by number of flags tends to be persistent for most sectors, though seems a little more visible in cyclical sectors. This may be because the several of the more cyclical "red flags" (such as Margins & ROIC and Days of Inventory & Receivables) affect those sectors more and may allow for greater contrast. Some "red flag" groups which were prominent early on, such as "Earnings Distortion" and "Amendments", mostly trended lower, while the prevalence of "Asset Turnover" warnings increased steadily over the period.

Returns broadly become more negative relative to the index over the period measured, which is surprising. We would expect the stocks doing better and worse than the Vanguard TMI to cancel each other out, not doing particularly better or worse in aggregate during any period. If later years are selected with the app filters, the difference in median and mean returns between higher and lower flag groups seems to be smaller. In the year 2000 (the beginning of our data), there was a more significant number of companies with 5-7 "red flags", and their under-performance was startling relative to the lower flag groups. We recall the post technology bubble period as being a golden age of stock picking. After 2004 or 2005, almost all red flag groups under-performed the TMI, which we struggle to explain. Interestingly, 2020 seems to be the only year when everything reversed, and high flag actually outperformed the TMI and lower flag groups, in proportion to the number of "red flags". Certainly, when Mr. Gulliver wrote his piece, the difference was more striking, so maybe more market observers took note than we might have thought and arbitraged the effect away.

# Thoughts about Data and Modeling {#modeling-and-data-thoughts}

By collapsing the ratios down into boolean "red flags" using arbitrarily thresholds, we lost a lot of data and are not able to allow variables to interact. We are also treating all of the variables equally, when some may be more important or may have more importance in combination with others. New Constructs was kind enough to share their data, but that was on a temporary basis, so this is beyond what we can achieve in this project, but taking these into account might give a better signal of future performance.

The model proposed by *Revelations* was rules-based, aggregating a large number of weak boolean signals. It's power comes from simplifying and systematically taking more elements over a larger number of companies than most single analysts might consider at the same time. In addition to the problem with sparsity among the higher flag groups, it isn't clear if the relationship between the number of flags and returns should be linear, and the few very filings with a high number of flags seemed to have more than a linear impact on returns. The statistical modeling tools we have learned so far rely on finding a few highly significant numeric variables, so we are not sure how to model when the signal derives from the sum of many weak variables.

The nature of the data is also a consideration. We would like to measure the true impact of individual or collection of flags. The challenge is the longitudinal nature of the data, and the related sparsity of high flag data by period. This is a situation best addressed with a Bayesian mixture model, which might allow us to get a more confident posterior for the sparse flag groups by taking into account measurements from other periods and helping to gauge our confidence in the differences of means of the "red flag" groups. This is likely something we will attempt in the future.

# Next Steps {#next-steps}

There are so many possible next steps it is difficult to know where to start. The first place would be to find the price histories for the missing 1,600 companies. We have included mainly accounting ratios in our analysis so far, but we could also easily add variables from other data sources, such as NLP signals such as sentiment of the MD&A, management incentive alignment measures, short interest ratios or insider selling. At the moment out of almost 145,000 filings, we found only about 1% with five or more "red flags" among the annual and quarterly data, so it might be beneficial to take into consideration more risk factors. With our code in an R `{targets}` project, any of these additions would only take minutes to add to the app, so it becomes a living document.

In our next blog post in this series, we will add a series of filters of several themes including: notorious accounting collapses, meme stocks, high internally-generated intangible and companies which were present over the whole period. We might add some more visualizations to the app to enable users to explore these filters. Finally, we will attempt to build the hierarchical, longitudinal mixture model as described above in order to better estimate the true means of the returns of the "red flag" groups.
